...
function Importation() {
  ...
  // Charger les facilités
  useEffect(() => {
    fetch('http://localhost:5000/auth/type_projets', { credentials: 'include' })
      .then((res) => res.json())
      .then((data) => setFacilites(data))
      .catch((error) => console.error('Erreur lors du chargement des facilités :', error));
  }, []);
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      // Conversion dates Excel -> YYYY-MM-DD
      const processedData = jsonData.map((row) =>
        row.map((cell) => {
  ...
  const handleFinishEdit = () => { setEditMode(false); setEditFinished(true); };

  // -------------------------
  // Importation vers le backend
  // -------------------------
  const handleImportToDB = async () => {
    if (!file) {
      setImportStatus({ success: false, message: 'Veuillez sélectionner un fichier avant d\'importer.' });
          ...
          <div>
            ...
            {!file ? (
              ...
              <div className="mt-8">
                ...
                {importStatus && (
                  <p className={`mt-4 text-center font-semibold ${importStatus.success ? 'text-green-700' : 'text-red-600'}`}>
                    {importStatus.message}
                  </p>
                )}
              </div>
